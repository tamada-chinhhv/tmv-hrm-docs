...
4. **勤怠打刻** — メニュー **Attendance & Time** → **Attendance** → **Check in** / **Check out**（Web: ブラウザの **位置情報/GPS** を許可。モバイルアプリ: 拠点が WiFi 設定の場合は **会社 WiFi** に接続 — [セクション 8.1](#81-仕組み)）。
...
| **System Settings** | 休日・拠点・勤務シフト・ロール・権限割当・書類期限通知 | `/sysConfig/holidays`, `/sysConfig/locations`, `/sysConfig/settings`, `/sysConfig/roles`, `/sysConfig/assign`, `/settings/document-notifications` |
...
**位置情報による打刻:** システムは **拠点半径内の GPS** または **オフィス WiFi（BSSID 一致）** のいずれか一方で検証します。**Web** ではブラウザの **Location（位置情報）** 許可が必要です。ブラウザは WiFi BSSID を読めないため、Web 打刻は GPS のみです。**モバイル**（連携時）は端末から `wifi.ssid` と `wifi.bssid` を送信します。
...
> `EMPLOYEE_CREATE` / `EMPLOYEE_UPDATE` / `EMPLOYEE_DELETE` を持つ HR/Admin 向け。

### 4.1 新規従業員作成手順

1. 作成権限のあるアカウントでログインする。
2. **Organization** → **Employees**（`/org/employees`）— `EMPLOYEE_VIEW` が必要。
3. **Add employee** をクリック。
4. フォームを入力する（下表）。
5. **Username** を確認する（氏名から自動入力 — 保存前に編集可）。
6. 必要なら **Role** を選択する（未選択 = ロール未割当）。
7. **Save** / **Create** をクリック。
8. 従業員一覧に戻る。従業員コード（`EMP…`）は自動採番される。

**期待される結果:** 新従業員が一覧に表示され、Username と初期パスワード（= Username）でログインできる。

#### フォーム項目

| 項目 | 必須 | 形式 / 備考 |
|------|:----:|-------------|
| **Full name** | ○ (*) | 最大 100 文字。作成中に氏名を変えると Username が再提案される |
| **Email** | — | 正しいメール形式。既存と重複不可 |
| **Phone** | — | |
| **Citizen ID** | — | |
| **Department** | — | Position より先に選択 |
| **Position** | — | Department 選択後に有効化 |
| **Date of birth** | — | DatePicker — 保存形式 **YYYY-MM-DD**（例: 1990-05-15） |
| **Gender** | — | Male / Female / Other |
| **Address** | — | |
| **Dependent count** | — | 整数 0–99 |
| **Total leave days** | — | 数値 ≥ 0 |
| **Remaining leave days** | — | 数値 ≥ 0 |
| **Hire date** | ○ (*) | 既定は当日。**YYYY-MM-DD** |
| **Contract type** | — | Full-time、Probation など |
| **Employment status** | — | 既定 **ACTIVE**。ほか **INACTIVE** / **TERMINATED** |
| **Username** | ○ (*) | 氏名から自動。**保存前**のみ編集可 |
| **Role** | — | 例: `EMPLOYEE`, `HR_MANAGER` — **`ADMIN` は `admin` のみ付与可** |
| **Attendance not required** | — | Admin のみ — Attendance Tracking グリッドと Excel 出力から除外 |
| **Direct manager** | — | 有効な従業員のみ |
| **Avatar** | — | 画像アップロード |

> **警告 — 日付:** UI はカレンダー選択。DB には **YYYY-MM-DD** で保存（DD/MM/YYYY ではない）。

> **警告 — Username:** 作成後は変更不可。保存前に必ず確認する。
...
| **Username** | 氏名から提案 — [セクション 3.2](#32-username-の自動生成ルール) |
| **Password** | Username と同じ（DB にハッシュ保存） |
...
| エラー | 原因 | 対処 |
|--------|------|------|
| **Username already exists** | Username 重複 | Username を編集（接尾辞追加）して再保存 |
| **Missing required fields** | 氏名・入社日・Username が空 | (*) 項目をすべて入力 |
| **Email already exists** | メール重複 | 別メールを使うか空欄にする |
| **Position must belong to department** | 選択部署に属さない Position | 部署/役職を選び直す |
| **Insufficient permissions** | `EMPLOYEE_CREATE` 不足 | Admin に **Permission Assignment** で権限付与を依頼 |

### 4.4 作成後の編集

1. **Organization** → **Employees** → 対象者を開く。
2. **Edit**（`/org/employees/{id}/edit`）。
3. 項目を更新（**Username** はロック）。
4. **Save**。

**自己編集プロフィール:** **Account**（`/account`）→ タブ **Information**（またはユーザーメニュー → **Account**）— 個人項目のみ（部署・ロール・Username は変更不可）。タブ **Settings**: テーマ色・フォント・ライト/ダーク — ユーザー単位で保存し、他端末ログイン時に同期。

**Admin の Reset password:** 従業員詳細 → **Reset password** → 確認 → パスワード = Username。**`admin` には適用されない**。

**Attendance not required:** Admin が従業員フォームのチェックを有効化 — その従業員は **Attendance Tracking** と勤怠 Excel から除外。ロール `ADMIN` は常に勤怠トラッキング対象外。
...
1. 従業員を編集する。
2. **Employment status** を **TERMINATED** または **INACTIVE** にする。
3. 保存する。

**Delete**（`EMPLOYEE_DELETE`）: DB から**完全削除** — 関連する勤怠・給与・カレンダーに影響しうる。可能な限りステータス変更を使う。**`admin` アカウントは削除不可**。
...
**Calendar** は従業員間の **会議・イベント** 予定用です。忙しい時間の確認、会議作成、参加者招待、変更時のアプリ内通知ができます。

**混同しないもの:**

- **Attendance Tracking** の月次勤怠グリッド。
- **Holiday Configuration** の会社休日設定。

**スケジュールカレンダー上のイベント種別:**

| 種別 | 説明 |
|------|------|
| **Meeting / event** | タイトル、時刻、場所、主催者、参加者 |
| **Recurring series** | 稼働日、週次曜日、または選択日付 |

> Calendar ページの凡例チップ（Meeting / Leave / Holiday）は参考表示です。時間グリッドに出るのは主に **会議** のみ。休暇・祝日は他モジュールで管理します。

### 5.2 カレンダーの見方

1. **Calendar** メニュー → `/calendar`。
...
1. **Title** — 必須。
2. **Participants** — 自分を含める。氏名で同僚を追加（有効な全従業員）。
3. **Date**、**Start time**、**End time** — 終了は開始より後。
4. **Location** — 任意。
5. **Recurrence** — 任意（[セクション 5.4](#54-繰り返し)）。
6. **Save**。
...
作成時に **Recurrence** を有効化（フォーム上での繰り返しルール編集は非対応 — 後から occurrence を編集/削除）。

| モード | 意味 |
|--------|------|
| **Working days** | 稼働日に繰り返し（休日設定の会社休日を除く） |
| **Weekly weekdays** | 毎週の選択曜日（月–日） |
| **Selected dates** | 特定日付を選択 |

表示週から約 **12 週間** 分の occurrence を生成（シリーズに終了日がない場合は延長あり）。

### 5.5 権限

原則: **作成者が管理**。**招待者は Leave 可**。**`CALENDAR_VIEW`**（seed/migrate 後の `EMPLOYEE` 既定）があれば他人のカレンダーを **閲覧** して会議調整できる。メニュー **Calendar**（`/calendar`）には `CALENDAR_VIEW` が必要。
...
| **Leave meeting** | 可 | イベントを消さずに辞退 — **理由必須**（主催者へ送信） |
| 追加招待 | **不可** | リストは主催者が管理 |
| Outlook 風 Accept/Decline | **不可** | **Leave meeting** + 通知を使用 |
...
| 他人の会議の編集/削除 | **可**（`CALENDAR_EDIT_ANY` が必要） | `ADMIN` に既定付与。他ロールは **Roles & permissions** で付与 |
| カレンダー上の「全社員表示」 | 可（任意） | `CALENDAR_MANAGE` — **Calendar** ページのスイッチ |
...
| 削除 | 詳細 → **Delete** → **単一** または **シリーズ全体** |
| 辞退 | 詳細 → **Leave meeting** → 理由入力 → 確認 |
...
| `HR_MANAGER` | HR Manager | HR 担当 | ロールは存在。**Admin が権限を付与**（seed では未割当） |
...
- **HR** = 通常 `HR_MANAGER` + Admin が付与した権限。
...
|------|:-----:|:---:|:-------:|:--------:|
| 従業員作成 | 可 | 可* | 不可** | 不可 |
| 従業員更新 | 可 | 可* | 不可** | Account → Information（限定） |
| 従業員削除 | 可 | 可* | 不可 | 不可 |
| 従業員閲覧 — 全社 | 可 | 可* | 不可 | 不可 |
| 従業員閲覧 — チーム | 可 | 可* | 可*** | 不可 |
| 従業員閲覧 — 自分のみ | 可 | 可 | 可 | 可 |
...
| どこで？ | **Organization → Employees** → Create/Edit → **Role** フィールド |
| 複数ロール？ | **不可** — 従業員あたり 1 ロール |
| **ADMIN** ロールの付与？ | Username **`admin` のみ** |
| **ADMIN** ロール権限の編集？ | Username **`admin` のみ** — システムは常に ADMIN に全権限を付与 |
| 権限の割当は？ | **System Settings → Permission Assignment**（`/sysConfig/assign`） |

**ロール権限の手順:**

1. **System Settings → Roles** — ロールの作成/閲覧（`ROLE_VIEW` / `ROLE_MANAGE`）。
2. **System Settings → Permission Assignment** — ロールを選択 → 権限にチェック → Save。
...
| `ATTENDANCE_EXPORT` | 労働時間詳細 Excel 出力（Attendance Tracking） |
...
| `LEAVE_DELETE_APPROVED` | **Leave Approvals** 上の **承認済み**申請削除（既定: ADMIN）。`LEAVE_APPROVE` / `LEAVE_APPROVE_MANAGED` の承認者も、決定可能な **APPROVED** 行を削除可 |
| `CALENDAR_VIEW` | カレンダー閲覧、自分のイベント作成/編集 |
| `CALENDAR_MANAGE` | 全社カレンダー管理スイッチ |
| `CALENDAR_EDIT_ANY` | 他従業員が所有するカレンダーイベントの編集/削除（既定: ADMIN） |
| `DOCUMENT_VIEW` | 閲覧 + 自分の従業員書類のセルフ CRUD；HR はすべて閲覧可 |
| `DOCUMENT_MANAGE` | 組織全体の書類 CRUD + 期限切れ通知ルールの設定 |
| `PAYROLL_VIEW` | 給与明細閲覧 |
...
> **システム外観**（全社既定）: `app_settings` に保存 — Admin が **System Settings → Settings** で設定。API `GET/PATCH /settings/appearance`（`APPEARANCE_*`）。**ログイン**と**ログアウト後**は常にシステム外観（`GET /settings/public/appearance`）。  
> **個人外観:** ログイン済みユーザー — **Account** → タブ **Settings**。`GET/PATCH /auth/me/appearance`。ユーザーが保存した場合のみ上書き（`appearance_customized = true`）。  
...
- Documents: `/account?tab=documents`
...
| 権限 | できること |
|------|------------|
| `DOCUMENT_VIEW` | 閲覧 + 自分の従業員書類のセルフ CRUD（`/account?tab=documents`）；HR はすべて閲覧可 |
| `DOCUMENT_MANAGE` | 組織全体の CRUD（任意の書類）、PDF アップロード、通知ルールの設定 |

**作成（HR）:** Organization → Documents → Add → Employee または Company を選択 → PDF をアップロード（氏名＋生年月日で従業員照合や有効期限の自動入力がある場合あり）→ 期限前リマインド日数（1/3/7/30、既定 30）を設定、または **No expiry date** にチェック → Save。

**従業員セルフサービス:** **Account → Documents** — **自分の**従業員書類のみ作成・編集・削除可（会社書類は不可。通知ルールは既定を使用）。

**通知先:** Settings → Document notifications（`/settings/document-notifications`）— 対象部署と通知受信者を選択。対応オプションが有効な場合、書類所有者も通知を受け取れます。平日 07:00（ベトナム時間）の cron が選択日にリマインドを送信。期限切れ書類は更新/削除まで毎日リマインド。**無期限**の書類はリマインダー cron の対象外です。
...
- **給与期間（`PayrollPeriod`）:** 既定は **Open**。HR が **Payroll** 上で **Lock period**（`PayrollPeriodControls`）→ ステータス **Locked**。ロック中は明細の作成/編集/インポート/コピー不可（API `PAYROLL_PERIOD_LOCKED`）。閲覧と Excel 出力は可能。**Unlock** には `PAYROLL_MANAGE` または `PAYROLL_PERIOD_LOCK` が必要（解除時に理由必須）。期間ロックは**給与操作のみ**を止めます — 勤怠・休暇は引き続き編集可（[セクション 10.4](#104-月末照合hr) 参照）。

### 7.5 System Settings

- **Holiday Configuration:** `HOLIDAY_CONFIG_VIEW` / `HOLIDAY_CONFIG_EDIT` — `/sysConfig/holidays`。
- **Office Locations:** `LOCATION_VIEW` / `LOCATION_MANAGE` — `/sysConfig/locations`。各 **active** 拠点は **GPS** または **active WiFi が 1 件以上**必須（GPS のみ / WiFi のみ / 両方可）。[セクション 7.5.1](#751-拠点設定gps--wifi) 参照。
- **システム外観:** `APPEARANCE_VIEW` / `APPEARANCE_EDIT` — **System Settings → Settings**（`/sysConfig/settings`、**Appearance** アコーディオン）。個人カスタム未設定のユーザーとログイン画面に適用。
- **勤務シフト（全社）:** `WORK_SHIFT_VIEW` / `WORK_SHIFT_EDIT` — 同ページ `/sysConfig/settings`、**Work shift** アコーディオン。
...
**パス:** `/sysConfig/locations` — **Branch configuration** ダイアログ。

| 項目 | 説明 |
|------|------|
| **Active** | 拠点の有効/無効。active 拠点は GPS または active WiFi ≥1 が必須。 |
| **GPS** | **Enable GPS** → 緯度・経度・半径（m）。GPS を無効にするとサーバー上の座標がクリアされる。 |
| **WiFi** | アクセスポイントごと: **SSID**（表示名）+ **BSSID**（AP の MAC、必須）。同一 SSID を複数 AP で共有可。ネットワークごとの **Active** スイッチ。 |
| **Detect current WiFi** | `GET /office-locations/wifi/current`（`LOCATION_MANAGE`）。**backend が動いているマシン**の WiFi を読む（Windows `netsh` / Linux `nmcli`）— オフィス網に接続した PC から管理者が設定する用途。 |
...
| 項目 | 内容 |
|------|------|
| 方式 | **Web** — Check in/out + **GPS**。**モバイル API** — GPS および/または **WiFi**（`wifi.bssid`）。ハードウェア打刻機は**なし**。 |
...
| 遅刻・早退 | 承認済み **`LATE_ARRIVAL`** / **`EARLY_DEPARTURE`** が評価閾値と **credit 分**（実打刻＋申請カバー分、昼休み重複なし）を調整。承認は打刻時刻を**上書きしない** — 従業員は通常どおり打刻する |
| 再打刻 | 既に時刻がある場合の 2 回目 Check in/out は既存レコードを返す（idempotent）。WiFi 打刻に GPS は不要 |
...
| Attendance Tracking の対象 | 勤怠必須の従業員 — ロール `ADMIN` と **Attendance not required** の従業員は除外 |
| 勤務シフト | **全社既定** を `/sysConfig/settings` で設定 — 従業員別ロスターなし。[8.7](#87-勤務シフトと勤務スケジュールtask-09) 参照 |

> **重要:** **LATE_EARLY** は設定された **勤務シフト**（開始/終了、grace、昼休み）と **期待労働分**（`workUnitLabel`）で評価され、固定 9h/540 分ルールではありません。承認済み **遅刻/早退申請**がある場合、閾値は **承認された** 到着/退勤時刻を使い、**credit 分** = 実労働＋申請カバー分（昼休みの二重計上なし）。例: シフト 08:00–17:00、昼休み 60 分、09:30 までの遅刻承認、打刻 09:30–17:00 → **WORK**（7.5h + 1.5h credit）。打刻 10:00–17:00 → **LATE_EARLY**（承認に対し 30 分遅刻。シフト+grace との比較ではない）。

#### 打刻方式とジオフェンス規則

| 方式 | HRM で利用可？ | 詳細 |
|------|:--------------:|------|
| **Web 自己打刻** | **あり** | `/time/attendance` で Check in / Check out。クライアントは GPS `location`（緯度/経度）を送信。 |
...
| **シフト設定** | System Settings | `work_shift_start_time`, `work_shift_end_time`, `grace_minutes`, `work_shift_lunch_break_minutes`。 |

| Check-in | Check-out | 合計分 | ステータス | Day モードグリッド |
|----------|-----------|--------|------------|-------------------|
| 08:00 | 17:00 | 540 | **WORK** | `1`（緑） |
| 08:15 | 17:15 | 540 | **WORK** | `1` |
| 08:00 | 16:30 | 510 | **LATE_EARLY** | `1`（黄） |
| 09:00 | 17:00 | 480 | **LATE_EARLY** | `1`（黄） |
| 08:00 | *(未 Check-out)* | — | **FORGOT_CLOCK_IN** または **WORK**（ケースによる） | `F` または Check-in のみ |
...
**再打刻:** 時刻保存後の 2 回目 Check in/out は既存レコードを返す（idempotent）。WiFi 打刻に GPS は不要。
...
| 業務タイムゾーン | **`Asia/Ho_Chi_Minh`**（UTC+7） |
| 勤怠の「今日」 | ベトナム日付 |
| 表示する Check-in/out | ベトナム時間（UTC スロット規約で保存） |

### 8.2 自己打刻

1. **Attendance**（`/time/attendance`）— **当月**のみ Check in/out ボタンが表示される。
2. 確認 → **Location** を許可 → GPS が設定拠点半径内であること（当日の承認済み **REMOTE_WORK** を除く）。WiFi 打刻は `wifi.bssid` を送るモバイルクライアント向け。
3. Check-in 後に Check-out。完了するとボタンは非表示。

**遅刻/早退申請が承認された日:** それでも **実際の** 時刻で打刻する。承認は **status の再計算のみ** — Check-in/out を事前設定しない。
...
1. **Attendance & Time** → **Attendance**（`/time/attendance`）、または **Overview** のクイック操作。
2. **当月**を選択。他月では打刻ボタンは非表示。
3. **Check in** → 確認 → ブラウザの **Location** を許可。
4. 成功すると緑のメッセージと、当日セルに Check-in 時刻が表示される。
5. Check-in 後に **Check out** → 同様に確認/位置検証。完了後ボタンは消える。
...
| 打刻忘れの状況 | システム記録 | 対処 |
|----------------|--------------|------|
| Check-in のみ | **FORGOT_CLOCK_IN**、または Check-out のみ欠けるケースでは WORK | 当日中に Check-out。**EARLY_DEPARTURE** / **ATTENDANCE_CORRECTION**。または HR に手動時刻を依頼。 |
| Check-out のみ | **FORGOT_CLOCK_IN** | Check-in を追加。**LATE_ARRIVAL** / **ATTENDANCE_CORRECTION**。または手動時刻。 |
...
| Admin | 同上 | 勤怠必須の**全**従業員（ロール `ADMIN` と **Attendance not required** は除外） |

グリッド記号: `1`/`8h` 出勤、`W` 週末、`H` 祝日、休暇コード、`F` 打刻忘れ、`A` 欠勤（チーム表示）、`-` 未来。

**従業員詳細:** `/time/attendance` で日付をクリックすると、Check-in/out、位置（あれば）、休暇/祝日の提案、権限があれば時刻編集フォームが開く。

**マネージャー手順:** **Attendance Tracking**（`/attendance-tracking`）を開く — `EMPLOYEE_VIEW` と直属/間接部下が必要。範囲は再帰的な `managerId` サブツリーのみ。氏名・月・1 つ以上の部署で絞り込み、`/attendance-tracking/{id}` で個人詳細を開く。
...
分類は Holiday Configuration → 承認済み休暇（グリッド上の REMOTE_WORK と ATTENDANCE_CORRECTION を除く）→ シフト/grace と承認済み遅刻/早退に対する勤怠評価の順。過去日に記録がなければ、チーム表示では ABSENT、一部の個人表示では FORGOT_CLOCK_IN。
...
- **エクスポート:** Attendance Tracking からの Excel `.xlsx` のみ — CSV/PDF なし。

**手動時刻 API:** `PATCH /attendance/manual-time`（`ATTENDANCE_MANUAL_UPDATE` 必須）。従業員詳細（`/attendance-tracking/{id}`）または個人カレンダーで **今日以前**の日付をクリックし、**Check-in / Check-out**（座標は任意）を入力して保存。Admin は有給や遅刻/早退申請がある日でも実打刻を編集可。
...
| 機能 | 利用可？ | 詳細 |
|------|:--------:|------|
| **月**でフィルタ | あり | Attendance / Attendance Tracking の MonthSelector。 |
| 従業員**名**でフィルタ | あり | Attendance Tracking。 |
| **部署**でフィルタ | あり | 複数部署を選択可。 |
| 週次フィルタ単体 | なし | 勤怠は月単位のみ。 |
| **Excel**（`.xlsx`）出力 | あり | Attendance Tracking: `GET /attendance/export-workingtime-detail`。`ATTENDANCE_EXPORT` 必須。 |
| **CSV / PDF** 出力 | **なし** | — |

Excel には従業員コード、氏名、各日の労働分、欠勤/遅刻/早退コード、残休暇日数などが含まれます。出力範囲はグリッドと同じ: Admin は全社、マネージャーは報告サブツリー。

### 8.6 権限マトリクス

| 操作 | 従業員 | マネージャー | HR/Admin |
|------|:------:|:------------:|:--------:|
| Check in/out | あり* | あり* | あり* |
| 自分のカレンダー | あり* | あり* | あり* |
...
> システムには **全社デフォルト勤務シフト** があり、従業員別ロスターはありません。**System Settings → Work shift**（`/sysConfig/settings`）で設定します。
...
| 労働単位プレビュー | **あり**: Settings 上で `(end − start − lunch)` |
| 従業員別/週次ロスター | **なし** |
| 承認付き 1 日シフト変更 | **なし** |
...
08:00–17:00、昼休み 60 分のシフトは **8 時間労働単位**になります。遅刻は `startTime + grace`（または承認済み `LATE_ARRIVAL` 時刻）より後。早退は `endTime − grace`（または承認済み `EARLY_DEPARTURE` 時刻）より前。固定休日は **Holiday Configuration** から。
...
| `LATE_ARRIVAL`, `EARLY_DEPARTURE` | なし | 承認時: 実打刻＋承認分から **status 再計算** — Check-in/out は**上書きしない** |
...
- **Total leave days**（`totalLeaveDays`）は参照値。
- **Remaining leave days**（`remainingLeaveDays`）は **`PAID_LEAVE`** 承認時のみ減算。

### 9.2 申請の作成

**Leave Requests**（`/time/leave`）→ フォーム: 種別、日付範囲、時刻（フォーム上の既定のみ 09:00–18:00）、理由、**Approver 1 名**（必須）。送信 → **PENDING**。承認者にアプリ内通知（メールなし）。

**残高超過:** `PAID_LEAVE` は**承認時**にブロック。送信時はブロックしない。

**申請フォーム詳細:**

1. **Attendance & Time** → **Leave Requests**（`/leave`）、または Attendance からクイック作成。
2. **Add** / 申請作成をクリック。
...
| 指標 | ソース | 意味 |
|------|--------|------|
| **Total leave days** | HR の従業員プロフィール | 参照のみ。自動減算されない。 |
...
**Leave Approvals** での **APPROVED** 行の **Delete**: **admin**（`ADMIN` ロール）、**指定承認者** / **直属上司**（`LEAVE_APPROVE` / `LEAVE_APPROVE_MANAGED`）、または `LEAVE_DELETE_APPROVED` を持つユーザー（`PAID_LEAVE` 残高を復元。安全な場合は `LATE_ARRIVAL` / `EARLY_DEPARTURE` / `ATTENDANCE_CORRECTION` の勤怠効果を戻す）。権限エラー: `LEAVE_DELETE_NOT_ALLOWED`（i18n）。
...
1. `LEAVE_REQUEST_CREATED` 通知、または **Leave Approvals**（`/leave-approvals`）を開く。
...
| HR/Admin はすべての申請を承認できる？ | いいえ。その申請で選ばれているか、従業員の代理作成でない限り不可。 |
...
| PENDING 編集 / 削除 | — | 通知なし |
| 未選択の HR/Admin が承認しようとする | — | 不可（403） |
...
| **Attendance Tracking** グリッド | `EMPLOYEE_VIEW` + スコープ | **Excel .xlsx**（`ATTENDANCE_EXPORT`） |
| ダッシュボード休暇/OT ウィジェット | 承認待ち件数、承認済み休暇日数。**本日の遅刻** = ライブ評価（読み取り専用、承認済み遅刻/早退を含む）。**OT 時間** = 当月の **承認済み** `OVERTIME` 合計 | — |
| 専用休暇 PDF/CSV | **なし** | — |

### 10.2 月次サマリーの見方

1. **マネージャー / HR:** **Attendance Tracking** を開き **月/年** を選択。
...
1. **Attendance Tracking** を開く。
2. 月を選び、必要なら部署/氏名でフィルタ。
3. **Export** / Excel 出力をクリック。
4. `.xlsx` をダウンロード。
...
| **給与期間** | 期間 **Open** — 明細の作成/編集/インポート/コピー可 | 期間 **Locked** — テーブル `payroll_periods`。**Payroll** 上の **Lock period** / **Unlock**。API `POST /payroll/periods/:year/:month/lock` と `unlock`（`PAYROLL_MANAGE` または `PAYROLL_PERIOD_LOCK`） |

給与期間をロックして給与計算する前に、下記チェックリストで **勤怠を照合**してください（[セクション 11.3](#113-月次)）。

#### 月末チェックリスト（HR）

- [ ] 締め対象月の **Attendance Tracking** を開く
...
- [ ] **Leave Approvals** の **PENDING** を確認 — 給与前に承認または却下
- [ ] 当月の承認済み **`PAID_LEAVE`** と **`remainingLeaveDays`** を整合
- [ ] 照合証跡として **Excel** を出力（ダウンロード時のファイルタイムスタンプ）
- [ ] 勤怠データが揃ったら **Payroll** へ
- [ ] **Payroll** で月/年を選び、明細確定後（または社内プロセスに従い公開前）に **Lock period**
...
4. Username/Password を配布し、全員に **パスワード変更**を依頼する。

### 11.2 日次

1. 勤怠の Check in/out。
2. 休暇の作成 / 承認。
3. 必要に応じて **Calendar** で会議を調整。
4. 例外対応（打刻忘れなど）。
...
3. **Payroll** で給与計算。完了後、その月を **Lock period**（`PAYROLL_MANAGE` または `PAYROLL_PERIOD_LOCK`）。
...
ログイン画面に自己復旧はありません。**HR または IT** にプロフィール上の **Reset password** を依頼してください。リセット後、パスワードは再び **Username** と同じになります — その後 [変更](#34-パスワード変更)してください。

**アカウントがロックされた — 誰に連絡する？**

専用の「アカウントロック」機能はありません。ログインできない場合: **Username**（メール/EMP コードではない）を確認 → HR にパスワード reset を依頼 → IT に連絡。プレースホルダ: HR _[メール/電話]_、IT _[メール/電話]_。

**Username を変更できる？**

**いいえ** — 従業員作成後、Username は永続です。
...
**08:15 に Check-in したのに「Late / Early」になるのはなぜ？**
...
確認: 参加者リストに入っているか、正しい **列** と **週/日** か、**Leave meeting** していないか。招待時は **ベル**通知も届くはずです。
...
**Calendar** → 会議を開く → **Leave meeting** → **理由**を入力 → 確認。

### 12.4 よくあるエラー

| エラー | 原因 | 対処 |
|--------|------|------|
| **Username already exists** | Username 重複 | 保存前に Username を変更（接尾辞を付ける等）。[セクション 4.3](#43-よくあるエラー) 参照。 |
| **Insufficient permissions** | 必要な権限がない | [セクション 6](#6-ロールと権限) を確認し、Admin に連絡。 |
...
| **Only the event organizer can modify** | 他人のイベントを編集しようとしている | **主催者**に編集を依頼するか、**Leave meeting**。 |
| **Insufficient remaining leave days** | `PAID_LEAVE` 承認が残高超過 | 却下するか、HR が従業員プロフィールの **Remaining leave days** を更新。 |
...
| **LEAVE_DELETE_NOT_ALLOWED** | admin・指定承認者・直属上司ではなく、`LEAVE_DELETE_APPROVED` もない | Leave Approvals からの削除は権限あるユーザーのみ。 |
