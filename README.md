...
| **給与** | 給与明細・税設定 | `/payroll` |
| **システム設定** | 休日・拠点・勤務シフト・ロール・権限割当・書類期限通知 | `/sysConfig/holidays`, `/sysConfig/locations`, `/sysConfig/settings`, `/sysConfig/roles`, `/sysConfig/assign`, `/settings/document-notifications` |
...
- **権限グループ / 権限割り当て:** `ROLE_VIEW` / `ROLE_MANAGE` — `/sysConfig/roles`, `/sysConfig/assign`。
