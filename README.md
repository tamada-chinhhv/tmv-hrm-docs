...
### 7.2.1 有効期限付き書類 (`/org/documents`)

従業員・会社の書類（PDF）と有効期限を管理し、期限前に通知します。権限: `DOCUMENT_VIEW` / `DOCUMENT_MANAGE`。通知先は `/settings/document-notifications`。詳細は [VI 版 7.2.1](README.vi.md#721-giấy-tờ-có-hạn-orgdocuments)。
